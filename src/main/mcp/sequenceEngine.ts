...
// 해당 코드에서 실행을 누르면 파이프라인을따라가야하는데 임의로 이동했다가 상태에따라 변하는 느낌이 들고 
// faild일 경우 붉은색으로 표시하게 ready running fail을 추가해서 붉은색으로 표시하고 테두리도 
// 붉은색으로표시하게 하고싶다 다크모드일때 글자들이 검은색이라 보이지 않는다